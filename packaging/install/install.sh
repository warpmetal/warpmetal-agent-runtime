...
server_id=""
bundle_dir=""
nested_private_procfs=preserve
while [ "$#" -gt 0 ]; do
  case "$1" in
...
case "$nested_private_procfs" in
  preserve|enable|disable) ;;
  *) echo "invalid_nested_private_procfs_mode" >&2; exit 2 ;;
esac
if [ "$nested_private_procfs" = enable ]; then
  case "$(uname -m)" in
    x86_64) ;;
...
if [ "$nested_private_procfs" != preserve ]; then
  ...
  case "$apparmor_requirement_status:$nested_private_procfs" in
