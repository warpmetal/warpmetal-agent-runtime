...
func readOpenMetadata(file *os.File) (fileMetadata, error) {
...
func readMetadata(path string) (metadata fileMetadata, err error) {
...
func comparePaths(leftPath, rightPath string) error {
	left, err := readMetadata(leftPath)
	if err != nil {
		return err
	}
	right, err := readMetadata(rightPath)
	if err != nil {
		return err
	}
...
func copyPath(sourcePath, destinationPath string) (err error) {
	...
	before, err := readOpenMetadata(source)
	if err != nil {
		return err
	}
	...
	after, err := readOpenMetadata(source)
	if err != nil {
		return err
	}
	...
	copied, err := readOpenMetadata(destination)
	if err != nil {
		return err
	}
